...
def main():
  ...
  x_axis = [num for num in range(50)]
  mlp_y_axis = [num * 100 for num in ml_perceptron_acc['iteration_accs']]
  nb_y_axis = [naive_bayes_acc['train_accuracy'] * 100 for _ in range(50)]

  plt.plot(x_axis, mlp_y_axis, label="MLP Accuracy")
  plt.plot(x_axis, nb_y_axis, label="Naive Bayes Accuracy")
  plt.title('Training Accuracies of Naive Bayes vs. Multi-Layer Perceptron')
